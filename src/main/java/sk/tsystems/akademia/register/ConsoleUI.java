...
public class ConsoleUI {
	...
	private JPADatabaseRegisterLoader JPALoader = new JPADatabaseRegisterLoader();
	...
	private enum SaveOption {
		DATABASE, DATABASEJPA, TEXTFILE, FILE, NONE
	};
	...
	/**
	 * Saves register to save destination selected by save option
	 */
	private void save() {
		if (saveDestination == SaveOption.DATABASE) {
			...
			saveTo(fileLoader);
		} else if (saveDestination == SaveOption.DATABASEJPA) {
			saveTo(JPALoader);
		} else {
			return;
		}
	...
	/**
	 * Executes saving register into file / database
	 * 
	 * @param loader
	 */
	...
	/**
	 * Loads every type of save register and shows which of them are available.
	 * Asks User in console to choose load destination
	 */
	private void loadRegisterFrom() {
		...
		if (i == 0) {
			out.println("There wasn't found any register. You have to create new one\n------------------------");
	...
	private void chooseSaveDestination() {
		...
		do {
			...
			case 1:
				...
				return;
			case 2:
				saveDestination = SaveOption.DATABASEJPA;
				save();
				return;
			case 3:
	...
	/**
	 * Creates new register, asks user for type of register
	 * 
	 * @return
	 */
	...
	/**
	 * Adds some persons into existing register. Optional method
	 */
